...
import heap.*;
import iterator.sm_join_assign_src.TupleUtilsException;
import global.*;
import java.io.*;
import java.lang.*;
...
public class TupleUtils
{
  ...
  public static int CompareTupleWithTuple(AttrType fldType,
	...
	case AttrType.attrReal:                // Compare two floats
	  ...
	  //////////////////////////////////////////////////
	 	case AttrType.attrInterval:         ///////////////////////////////////////////// compare two interval
	 		try {
	 			t1_in = t1.getIntervalFld(t1_fld_no);
	 			t2_in = t1.getIntervalFld(t2_fld_no);
	 	...
	 	if(t1_in.gets()<t2_in.gets() && t1_in.gete()>t2_in.gete()) return 2;
	 	if(t1_in.gets()>t2_in.gets() && t1_in.gete()<t2_in.gete()) return 1;	
	 	if(t1_in.gets()==t2_in.gets() && t1_in.gete()==t2_in.gete()) return 0;
	 	if(t1_in.gets()<t2_in.gets() && t1_in.gete()<t2_in.gete()&& t1_in.gets()<t2_in.gete())return 0;
	 	if(t1_in.gets()>t2_in.gets() && t1_in.gete()>t2_in.gete()&& t1_in.gets()>t2_in.gete())return 0;
	 	if(t1_in.gets()<t2_in.gets() && t1_in.gete()<t2_in.gete()&& t1_in.gets()>t2_in.gete()) return 3;
	 	if(t1_in.gets()>t2_in.gets() && t1_in.gete()>t2_in.gete()&& t1_in.gets()<t2_in.gete()) return 3;
